...
if __name__ == '__main__':
    ...
    for n in range(3,31,1):
         ...
         plt.contourf(X, Z, den_e.T, levels=levels, cmap='jet', norm=colors.Normalize(vmin=np.min(levels),vmax=np.max(levels)))
     ...
     #    cbar.set_label('$E_y$ [$m_ec\omega_0/|e|$]',fontdict=font2)        
         ...
         plt.subplots_adjust(left=0.12, bottom=0.12, right=0.98, top=0.98, wspace=0.05, hspace=0.05)
         fig = plt.gcf()
         fig.set_size_inches(10, 6.5)
         fig.savefig(to_path+'density_electron_'+str(n).zfill(4)+'.png',format='png',dpi=160)
         plt.close("all")
         print(to_path+'density_electron_'+str(n).zfill(4)+'.png')
