...
if __name__ == '__main__':
    ...
    for n in range(11,23,1):
         ...
         ey1  = data['Electric Field/Ez_averaged'].data/exunit*3.5
         ey2    = np.sum(ey1[:,179:181,:],axis=1)/2
     
         levels = np.linspace(-20,20, 41)
         ey[ey>np.max(levels)]=np.max(levels); 
         ey[ey<np.min(levels)]=np.min(levels);
         ey2[ey2>np.max(levels)]=np.max(levels); 
         ey2[ey2<np.min(levels)]=np.min(levels);
     
         plt.subplot(2,1,1)
         plt.contourf(X, Z, ey.T, levels=levels, cmap='BrBG_r', norm=colors.Normalize(vmin=np.min(levels),vmax=np.max(levels)))
...
#         plt.contour(X, Z, den_e.T,levels=[30.0], colors='white', linewidths=2.,origin='lower')
     ...
     #    plt.xlabel('X [$\mu m$]',fontdict=font)
         ...
         plt.xticks([-5,0,5,10,15,20,25,30,35],fontsize=0.001); 
         plt.yticks([-5,0,5],fontsize=font_size);
         plt.title('t='+str(round(time/1e-15,2))+' fs',fontdict=font)
         
         plt.subplot(2,1,2)
         plt.contourf(X, Z, ey2.T, levels=levels, cmap='BrBG_r', norm=colors.Normalize(vmin=np.min(levels),vmax=np.max(levels)))
         cbar=plt.colorbar(pad=0.01,ticks=[-20,0,20])
         cbar.ax.tick_params(labelsize=font2['size']) 
         cbar.set_label('$\overline{E}_z$ [TV/m]',fontdict=font2)        
...
#         plt.contour(X, Z, den_e2.T,levels=[30.0], colors='white', linewidths=2.,origin='lower')
         ...
         plt.xlabel('X [$\mu m$]',fontdict=font)
         plt.ylabel('Z [$\mu m$]',fontdict=font)
         plt.xticks([-5,0,5,10,15,20,25,30,35],fontsize=font_size); 
         plt.yticks([-5,0,5],fontsize=font_size);
     #    plt.title('t='+str(round(time/1e-15,2))+' fs',fontdict=font)
         ...
         fig.savefig(to_path+'wrap_ey_averaged_'+str(n).zfill(4)+'.png',format='png',dpi=160)
         plt.close("all")
         print(to_path+'wrap_ey_averaged_'+str(n).zfill(4)+'.png')
