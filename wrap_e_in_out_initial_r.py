...
if __name__ == '__main__':
  ...
  print(np.max(weight[:,0]),np.min(weight[:,0]))
  fig,host = plt.subplots()
  #plt.hist2d(yy[:,0], zz[:,0], bins=(50, 50), range=[[-8,8],[-8,8]], cmap=mycolormap, weights=weight[:,0], normed=False, norm=colors.Normalize(vmin=0, vmax=10))
  plt.hist2d(yy[:,0], zz[:,0], bins=(50, 50), range=[[-8,8],[-8,8]], cmap=mycolormap, weights=weight[:,0]/1.5e5,norm=colors.Normalize(vmin=0, vmax=100))
...
#  plt.Circle((0,0),1.8, color='k',fill=False,linestyle=':',linewidth=4,edgecolor='black')
  ...
  fig.savefig(to_path+'wrap_e_in_out_initial.png',format='png',dpi=160)
  plt.close("all")
