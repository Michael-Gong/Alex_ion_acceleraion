...
def processplot(n): 
  ...
  if 'Particles/Px/subset_Only_Ions0/Ion' not in data:
    return 0
  header=data['Header']
  time1=header['time']
  ...
  grid_x = grid_x[px > 0.05]
  theta = abs(theta[px > 0.05])
  px = px[px > 0.05]
...
#lt.figure(figsize=(100,100))
  ...
  fig.savefig(to_path+'proton_px_x'+str(n).zfill(4)+'.png',format='png',dpi=80)
  plt.close("all")
