...
#         plt.contour(X, Z, den_e.T,levels=[30.0], colors='white', linewidths=2.,origin='lower')
    ...
    plt.xticks([10,15,20],fontsize=font_size); 
    plt.yticks([-3,0,3],fontsize=font_size);
#    plt.title('t='+str(round(time/1e-15,2))+' fs',fontdict=font)

    
    plt.subplots_adjust(left=0.14, bottom=0.16, right=0.99, top=0.99, wspace=0.02, hspace=0.05)
    fig = plt.gcf()
    fig.set_size_inches(10, 4.5)
    fig.savefig('./wrap_ey_ave_t260.png',format='png',dpi=160)
    plt.close("all")
    print('./wrap_ey_ave_t260.png')
