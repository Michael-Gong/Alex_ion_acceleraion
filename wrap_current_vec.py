...
if __name__ == '__main__':
  ...
  #X, Y = np.meshgrid(x, y)
              ...
              j_xy = j_xy[j_xy > threshold_quiver]
              nor = (jx**2+jy**2+1.)**0.5
              plt.quiver(X, Y, -jx/nor*0.1, -jy/nor*0.1, j_xy, cmap=mycolormap, pivot='mid')
              #### manifesting colorbar, changing label and axis properties ####
              cbar=plt.colorbar(pad=0.01,ticks=[0, 10,20,30,40])
              cbar.set_label('$j_{xy}$ [$I_A/\lambda^2$]',fontdict=font)
              ...
              if np.min(j_xz) == np.max(j_xz):
                  continue
              levels = np.linspace(0, eee, 40)
              plt.contourf(X, Z, j_xz.T, levels=levels, norm=mcolors.Normalize(vmin=0, vmax=eee), cmap=mycolormap)
              #### manifesting colorbar, changing label and axis properties ####
              cbar=plt.colorbar(pad=0.01,ticks=[0, 10,20,30,40])
              cbar.set_label('$j_{xz}$ [$I_A/\lambda^2$]',fontdict=font)
              cbar.ax.set_yticklabels(cbar.ax.get_yticklabels(),fontsize=15)        
              plt.xlabel('X [$\mu m$]',fontdict=font)
              ...
              if np.min(j_xy) == np.max(j_xy):
                  continue
              levels = np.linspace(0, eee, 40)
              plt.contourf(X, Y, j_xy.T, levels=levels, norm=mcolors.Normalize(vmin=0, vmax=eee), cmap=mycolormap)
              #### manifesting colorbar, changing label and axis properties ####
              cbar=plt.colorbar(pad=0.01,ticks=[0, 10,20,30,40])
              cbar.set_label('$j_{xy}$ [$I_A/\lambda^2$]',fontdict=font)
              cbar.ax.set_yticklabels(cbar.ax.get_yticklabels(),fontsize=15)        
              ...
              fig.savefig('./J_cut=0_vec'+str(n).zfill(4)+'.png',format='png',dpi=100)
              plt.close("all")
