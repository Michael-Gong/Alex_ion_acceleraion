...
if __name__ == '__main__':
  ...
  #X, Y = np.meshgrid(x, y)
              ...
              plt.contourf(X, Y, j_xy.T, levels=levels, norm=mcolors.LogNorm(vmin=levels.min(), vmax=levels.max()), cmap=mycolormap)
              #### manifesting colorbar, changing label and axis properties ####
              cbar=plt.colorbar(pad=0.01,ticks=[1e-1,1,1e1,1e2])
...
##              plt.quiver(X, Y, 0.1*jx, 0.1*jy, color='red')
              ...
              if np.min(j_xz) == np.max(j_xz):
                  continue
              plt.contourf(X, Y, j_xz.T, levels=levels, norm=mcolors.LogNorm(vmin=levels.min(), vmax=levels.max()), cmap=mycolormap)
              #### manifesting colorbar, changing label and axis properties ####
              cbar=plt.colorbar(pad=0.01,ticks=[1e-1,1,1e1,1e2])
              ...
              fig.savefig('./J_cut=0_log'+str(n).zfill(4)+'.png',format='png',dpi=100)
              plt.close("all")
