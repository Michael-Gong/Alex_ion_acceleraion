...
if __name__ == '__main__':
  from_path='./uniform_a190_n30_e_div_2/2_'
  to_path  =from_path
  
  x_in = np.loadtxt(from_path+'xx3d_uni.txt')
  y_in = np.loadtxt(from_path+'yy3d_uni.txt')
  z_in = np.loadtxt(from_path+'zz3d_uni.txt')
  w_in = np.loadtxt(from_path+'ww3d_uni.txt')
  ...
  plt.hist2d(yy[:,0], zz[:,0], bins=(50, 50), range=[[-8,8],[-8,8]], cmap=mycolormap, weights=weight[:,0]/1.5e5, normed=False, norm=colors.Normalize(vmin=0, vmax=100))
  cbar=plt.colorbar(pad=0.01)
  cbar.set_label(r'$dN/(dydz)$'+' [A.U.]',fontdict=font)
  cbar.ax.set_yticklabels(cbar.ax.get_yticklabels(),fontsize=20)

  x=np.linspace(-1.8,1.8,101)
  plt.plot(x,(1.8**2-x**2)**0.5,':r',linewidth=4.5)
  plt.plot(x,-(1.8**2-x**2)**0.5,':r',linewidth=4.5)
...
#  plt.Circle((0,0),1.8, color='k',fill=False,linestyle=':',linewidth=4,edgecolor='black')
  ...
  plt.xticks([-8,-4,0,4,8],fontsize=25); plt.yticks([-8,-4,0,4,8],fontsize=25);
  ...
  fig.savefig(to_path+'wrap_e_in_out_initial_r.png',format='png',dpi=160)
  plt.close("all")
