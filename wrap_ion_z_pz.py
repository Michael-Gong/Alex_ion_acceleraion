...
if __name__ == '__main__':
  ...
  img = ax.scatter(grid_z, (pz/px), color='k', s=0.3, edgecolors='None', alpha=0.4)


  x  = np.loadtxt(from_path+'ez_lineout_x.txt')
  z  = np.loadtxt(from_path+'ez_lineout_z.txt')
  ex = np.loadtxt(from_path+'ez_lineout_data_'+str(n).zfill(4)+'.txt')
  condition_x = (x>= 18) & (x<=21)
  ex = np.sum(ex[condition_x,:],axis=0)/np.size(ex[condition_x,0])
#      x  = x[ (x>= serial_range[i][0]) & (x<=serial_range[i][1])]
  ax2.plot(z,ex, '--', linewidth=3, color='red', label="Ez")

#  fig.colorbar(img,cax=cax,label='time [fs]', ticks=[200,240,280,320,360])
  ax.set_xlim(-2.5,2.5)
  ax.set_ylim(-0.45,0.45)
  ax.set_xlabel(r'$Z\ [\mu m]$',fontdict=font)
...
#  plt.text(-100,650,' t = '++' fs',fontdict=font)
  ...
  ax2.set_ylabel('$\overline{E}_z$ [$m_ec\omega/|e|$]',fontdict=font,color='r')
...
#lt.figure(figsize=(100,100))
  ...
  fig.savefig('./wrap_ion_z_pz.png',format='png',dpi=160)
  plt.close("all")
  print('finised ')
