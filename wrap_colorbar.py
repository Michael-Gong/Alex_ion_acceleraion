...
if __name__ == '__main__':
    ...
    cb1 = matplotlib.colorbar.ColorbarBase(ax1, cmap=cmap_wyr, norm=colors.LogNorm(vmin=5e7,vmax=1e9), 
                                    orientation='horizontal',ticks=[1e8,1e9])
    cb1.ax.xaxis.set_label_position('top'); cb1.ax.xaxis.set_ticks_position('top')
    cb1.ax.tick_params(labelsize=font_size) 
    cb1.set_label(r'dN/(d$x$d$p_x$)'+' [A.U.]',fontdict=font)
    plt.subplots_adjust(top=0.98, bottom=0.13, left=0.15, right=0.85, hspace=0.10, wspace=0.05)
    fig = plt.gcf()
    fig.set_size_inches(7, 2)
    plt.show()
    fig.savefig('./colorbar_fig/phase_px_x_wyr.png',format='png',dpi=160, transparent=True)
    plt.close("all")

    fig = plt.figure()
    ax1 = fig.add_axes([0.1, 0.2, 0.5, 0.15])
    cb1 = matplotlib.colorbar.ColorbarBase(ax1, cmap='bwr', norm=colors.Normalize(vmin=-20,vmax=20), 
                                    orientation='horizontal',ticks=[-20,0,20])
    cb1.ax.xaxis.set_label_position('top'); cb1.ax.xaxis.set_ticks_position('top')
    cb1.ax.tick_params(labelsize=font_size) 
    cb1.set_label(r'$E_z$'+' [A.U.]',fontdict=font)
    plt.subplots_adjust(top=0.98, bottom=0.13, left=0.15, right=0.85, hspace=0.10, wspace=0.05)
    fig = plt.gcf()
    fig.set_size_inches(7, 2)
    plt.show()
    fig.savefig('./colorbar_fig/ey_field.png',format='png',dpi=160, transparent=True)
    plt.close("all")
