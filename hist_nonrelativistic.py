  ...
  for n in np.arange(13,16):
    for m in np.arange(27,28): 
      ...
      if set_relativistic == 0: 
          ...
          pz = data['Particles/Py/subset_Only_Ions0/Ion'].data
      ...
      plt.hist(ek_1, **kwargs)
      kwargs = dict(histtype='stepfilled', alpha=0.3, normed=None, color='r', bins=300, range=(0,600), weights=ww_2/2)
      plt.hist(ek_2, **kwargs)
    ...
    #        plt.text(250,6e9,'t='+str(round(time/1.0e-15,0))+' fs',fontdict=font)
      ...
      fig.savefig('./hist_nonrelativistic_'+str(n).zfill(4)+'_'+str(m).zfill(4)+'.png',format='png',dpi=160)
      plt.close("all")
